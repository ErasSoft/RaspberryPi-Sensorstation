...
def install():
	errorlevel = 9
	try:
		errorlevel = subprocess.call("install/ds18b20.sh")
	except:
	...
	if (errorlevel == 0):
		...
		showinfo("Installation erfolgreich", "Die Installation war erfolgreich! Um die Installation abzuschliessen muss der Raspberry Pi neu gestartet werden!")
	if (errorlevel == 1):
...
def test():
	...
	if (testGPIO(gp1) ):
		...
		writeToLogfile("LOG: %s - Test measurement of the sensor %s to GPIO number %s: %s" % (getTimestamp(), sensor, gp1, sensor_ds18b20) )
		showinfo('Test-Messung', sensor_ds18b20)
...
def csv():
	...
	cursor = connection.cursor()
	try:	
		cursor.execute("SELECT * FROM ds18b20")
		csvdata = "data/ds18b20.csv"
		# Datei neu erstellen
		f = open(csvdata, "w")
		f.close()
